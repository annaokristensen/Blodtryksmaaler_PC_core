			...
			do
			{
				while(!shallStop) //Skal kører så længe shallstop er true (skal ændres til start/stop knap på GUI)
				{
                    ...
                    foreach (string sample in holder)
                    {
						...
						BPMeasurementData_DTO s = new BPMeasurementData_DTO(Convert.ToInt32(input[0]), Convert.ToInt32(input[1]), Convert.ToInt32(input[2]), Convert.ToInt32(input[3]), Convert.ToDateTime(input[4])); //gemmer intput 1, 2, 3 i DTO
                        samplesList.Add(s);
                    }
                    return samplesList;
                }
